...
const worker=new Worker(new URL('./slam-worker-build-20260925-27.js?v=20260925.27',import.meta.url),{type:'classic'});
...
function renderResult(m){currentRawPose=m.pose;currentMatrix=m.matrix;mapMode=m.mapMode||mapMode;const p=relativePose(m.pose,m.matrix);$('trackingState').textContent=m.state;$('slamHz').textContent=`${m.slamHz.toFixed(1)} Hz`;$('computeMs').textContent=`${m.computeMs.toFixed(1)} ms`;$('dropped').textContent=String(dropped);$('inliers').textContent=String(m.inliers);$('rmse').textContent=Number.isFinite(m.rmse)?`${(m.rmse*1000).toFixed(1)} mm`:'—';$('keyframes').textContent=String(m.keyframes);$('loops').textContent=String(m.loops);$('relocalized').textContent=String(m.relocalized);$('mapMatches').textContent=String(m.mapMatches||0);$('mapRejects').textContent=String(m.mapRejects||0);$('mapRepairs').textContent=String(m.mapRepairs||0);$('localAnchors').textContent=String(m.localAnchors||0);$('localAnchorRejects').textContent=String(m.localAnchorRejects||0);$('localCorr').textContent=m.localLocked?`${(m.localCorrectionM*1000).toFixed(1)} mm / ${(m.localCorrectionDeg||0).toFixed(2)}°`:'—';$('localRef').textContent=m.localLocked?(m.localTrusted?'TRUSTED MAP':'RECENT KF'):'—';$('localAnchorFrame').textContent=m.localAnchorFrame?String(m.localAnchorFrame):'—';$('trustedFrame').textContent=String(m.lastTrustedFrame||0);$('t265Fusion').textContent=m.t265Bridge?`BRIDGE · ${confidenceName(m.t265Confidence)}`:(m.t265Used?`USED · ${confidenceName(m.t265Confidence)}`:(m.t265Available?`FALLBACK · ${confidenceName(m.t265Confidence)}`:'NO POSE'));$('t265Delta').textContent=Number.isFinite(m.t265DeltaDeg)?`${m.t265DeltaDeg.toFixed(2)}° / ${Number.isFinite(m.t265DeltaM)?(m.t265DeltaM*1000).toFixed(1):'—'} mm`:'—';$('t265IcpDiff').textContent=Number.isFinite(m.t265IcpDisagreeDeg)?`${m.t265IcpDisagreeDeg.toFixed(2)}° / ${Number.isFinite(m.t265IcpTranslationDiffM)?(m.t265IcpTranslationDiffM*1000).toFixed(1):'—'} mm`:'—';$('t265Counts').textContent=`${m.t265UsedFrames||0} / ${m.t265FallbackFrames||0}`;$('t265Retry').textContent=`${m.t265RetryFrames||0} · bridge ${m.t265BridgeFrames||0}`;if($('mapT265Init'))$('mapT265Init').textContent=m.mapT265Init?'AVAILABLE':'NONE';if($('lastMapAnchor'))$('lastMapAnchor').textContent=m.lastMapAnchorFrame?String(m.lastMapAnchorFrame):'—';if($('lastMapT265Near'))$('lastMapT265Near').textContent=Number.isFinite(m.lastMapT265NearM)?`${m.lastMapT265NearM.toFixed(3)} m / ${Number.isFinite(m.lastMapT265NearDeg)?m.lastMapT265NearDeg.toFixed(1):'—'}°`:'—';if($('lastMapT265Abs'))$('lastMapT265Abs').textContent=Number.isFinite(m.lastMapT265AbsM)?`${m.lastMapT265AbsM.toFixed(3)} m / ${Number.isFinite(m.lastMapT265AbsDeg)?m.lastMapT265AbsDeg.toFixed(1):'—'}°`:'—';$('staticState').textContent=m.state==='STATIC'?'LOCKED':`${m.staticStreak||0}/4`;$('staticDepth').textContent=Number.isFinite(m.staticMedianMm)?`${m.staticMedianMm.toFixed(1)} / ${m.staticP90Mm.toFixed(1)} mm`:'—';$('staticRatio').textContent=`${((m.staticCloseRatio||0)*100).toFixed(0)}%`;$('staticOdom').textContent=Number.isFinite(m.staticOdomMm)?`${m.staticOdomMm.toFixed(1)} mm / ${m.staticOdomDeg.toFixed(2)}°`:'—';$('staticRgb').textContent=Number.isFinite(m.staticRgbMotionPx)?`${m.staticRgbInliers||0}/${m.staticRgbMatches||0} · ${m.staticRgbMotionPx.toFixed(2)} px / ${m.staticRgbAngleDeg.toFixed(2)}°`:'—';$('staticNoise').textContent=Number.isFinite(m.depthNoiseMedMm)?`${m.depthNoiseMedMm.toFixed(1)} / ${m.depthNoiseP90Mm.toFixed(1)} mm (${m.depthNoiseSamples||0})`:'—';$('staticGateDepth').textContent=Number.isFinite(m.staticDepthMedLimitMm)?`${m.staticDepthMedLimitMm.toFixed(1)} / ${m.staticDepthP90LimitMm.toFixed(1)} mm`:'—';$('rgbFresh').textContent=m.staticFreshRgb?'NEW':'HOLD';$('staticLatch').textContent=m.staticLatched?'LATCHED':'FREE';$('staticMove').textContent=`${m.staticMoveScore||0} · exit ${m.staticExitStreak||0}/2`;if($('staticT265Latch'))$('staticT265Latch').textContent=Number.isFinite(m.staticT265LatchM)?`${(m.staticT265LatchM*1000).toFixed(1)} mm / ${Number.isFinite(m.staticT265LatchDeg)?m.staticT265LatchDeg.toFixed(2):'—'}°`:'—';if($('staticSources'))$('staticSources').textContent=m.staticMotionSources||'—';if($('staticMap'))$('staticMap').textContent=`${m.staticMapChecks||0} checks / ${m.staticMapLocks||0} locks`;$('mapModeValue').textContent=mapMode==='mapping'?'BUILD':'LOCALIZE';$('rgbFeatures').textContent=String(m.rgbFeatures||0);$('rgbMatches').textContent=String(m.rgbMatches||0);$('rgbInliers').textContent=String(m.rgbInliers||0);$('confirmHits').textContent=String(m.confirmHits||0);$('recoveryGate').textContent=m.needsMapRecovery?'MAP REQUIRED':'FREE';$('posX').textContent=fmtM(p.x);$('posY').textContent=fmtM(p.y);$('posZ').textContent=fmtM(p.z);$('roll').textContent=fmtD(p.roll);$('pitch').textContent=fmtD(p.pitch);$('yaw').textContent=fmtD(p.yaw);setBadge(m.state,(m.state==='TRACKING'||m.state==='T265_BRIDGE'||m.state==='RELOCALIZED'||m.state==='MAP_LOCKED'||m.state==='STATIC')?'live':(m.state==='LOST'||m.state==='SEARCHING_MAP'||m.state==='RECOVERY_CONFIRM'||m.state==='MAP_CONFIRM')?'warn':'idle');path=m.path||path;keyframePath=m.keyframePath||keyframePath;trajView.render(path,keyframePath);trajectoryRows.push([performance.now(),p.x,p.y,p.z,p.roll,p.pitch,p.yaw,m.state,m.rmse,m.inliers]);
  ...
  if((m.mapMatches||0)>lastLoggedMapMatches){log('INFO',`Map anchor accepted. total=${m.mapMatches} anchorKF=${m.lastMapAnchorFrame||0} correction=${(m.mapDriftM||0).toFixed(3)}m / ${(m.mapDriftDeg||0).toFixed(2)}deg RGB=${m.rgbInliers||0}/${m.rgbMatches||0} t265Near=${Number.isFinite(m.lastMapT265NearM)?m.lastMapT265NearM.toFixed(3):'na'}m/${Number.isFinite(m.lastMapT265NearDeg)?m.lastMapT265NearDeg.toFixed(1):'na'}deg t265Abs=${Number.isFinite(m.lastMapT265AbsM)?m.lastMapT265AbsM.toFixed(3):'na'}m/${Number.isFinite(m.lastMapT265AbsDeg)?m.lastMapT265AbsDeg.toFixed(1):'na'}deg`);lastLoggedMapMatches=m.mapMatches||0;setMapState(`MAP LOCK · ${(m.mapDriftM||0).toFixed(3)} m / ${(m.mapDriftDeg||0).toFixed(1)}° · RGB ${m.rgbInliers||0}/${m.rgbMatches||0}`,'ok');}
  ...
  const now=performance.now();if(now-lastHealthLogAt>3000){log('INFO',`Health: state=${m.state} camera=${depthFps.toFixed(1)}Hz slam=${m.slamHz.toFixed(1)}Hz compute=${m.computeMs.toFixed(1)}ms dropped=${dropped} inliers=${m.inliers} rmse=${fmtRmse(m.rmse)} RGBfeatures=${m.rgbFeatures||0} RGBmatch=${m.rgbInliers||0}/${m.rgbMatches||0} rejects=${m.mapRejects||0} confirm=${m.confirmHits||0}/${m.confirmRequired||2} recoveryGate=${m.needsMapRecovery?'MAP':'FREE'} repairs=${m.mapRepairs||0} local=${m.localAnchors||0}/${m.localAnchorRejects||0} localRef=${m.localLocked?(m.localTrusted?'TRUSTED':'RECENT'):'na'} localKF=${m.localAnchorFrame||0} localCorr=${m.localLocked?`${(m.localCorrectionM*1000).toFixed(1)}mm/${(m.localCorrectionDeg||0).toFixed(2)}deg`:'na'} t265=${m.t265Bridge?'BRIDGE':m.t265Used?'USED':'FALLBACK'} conf=${m.t265Confidence||0} dR=${Number.isFinite(m.t265DeltaDeg)?m.t265DeltaDeg.toFixed(2):'na'}deg dT=${Number.isFinite(m.t265DeltaM)?(m.t265DeltaM*1000).toFixed(1):'na'}mm icpDiff=${Number.isFinite(m.t265IcpDisagreeDeg)?m.t265IcpDisagreeDeg.toFixed(2):'na'}deg/${Number.isFinite(m.t265IcpTranslationDiffM)?(m.t265IcpTranslationDiffM*1000).toFixed(1):'na'}mm t265Count=${m.t265UsedFrames||0}/${m.t265FallbackFrames||0} bridge=${m.t265BridgeFrames||0} mapT265=${m.mapT265Init?'Y':'N'} mapAnchor=${m.lastMapAnchorFrame||0} mapNear=${Number.isFinite(m.lastMapT265NearM)?m.lastMapT265NearM.toFixed(3):'na'}m/${Number.isFinite(m.lastMapT265NearDeg)?m.lastMapT265NearDeg.toFixed(1):'na'}deg mapAbs=${Number.isFinite(m.lastMapT265AbsM)?m.lastMapT265AbsM.toFixed(3):'na'}m/${Number.isFinite(m.lastMapT265AbsDeg)?m.lastMapT265AbsDeg.toFixed(1):'na'}deg staticMap=${m.staticMapChecks||0}/${m.staticMapLocks||0} trustedFrame=${m.lastTrustedFrame||0} static=${m.staticStreak||0}/4 blockDepthΔ=${Number.isFinite(m.staticMedianMm)?m.staticMedianMm.toFixed(1):'na'}/${Number.isFinite(m.staticP90Mm)?m.staticP90Mm.toFixed(1):'na'}mm close=${((m.staticCloseRatio||0)*100).toFixed(0)}% odom=${Number.isFinite(m.staticOdomMm)?m.staticOdomMm.toFixed(1):'na'}mm/${Number.isFinite(m.staticOdomDeg)?m.staticOdomDeg.toFixed(2):'na'}deg rgb=${m.staticRgbInliers||0}/${m.staticRgbMatches||0}:${Number.isFinite(m.staticRgbMotionPx)?m.staticRgbMotionPx.toFixed(2):'na'}px fresh=${m.staticFreshRgb?'Y':'N'} noise=${Number.isFinite(m.depthNoiseMedMm)?m.depthNoiseMedMm.toFixed(1):'na'}/${Number.isFinite(m.depthNoiseP90Mm)?m.depthNoiseP90Mm.toFixed(1):'na'}mm gate=${Number.isFinite(m.staticDepthMedLimitMm)?m.staticDepthMedLimitMm.toFixed(1):'na'}/${Number.isFinite(m.staticDepthP90LimitMm)?m.staticDepthP90LimitMm.toFixed(1):'na'}mm latch=${m.staticLatched?'Y':'N'} moveScore=${m.staticMoveScore||0} exit=${m.staticExitStreak||0}/2 t265Latch=${Number.isFinite(m.staticT265LatchM)?(m.staticT265LatchM*1000).toFixed(1):'na'}mm/${Number.isFinite(m.staticT265LatchDeg)?m.staticT265LatchDeg.toFixed(2):'na'}deg sources=${m.staticMotionSources||'na'} pose=[${p.x.toFixed(3)},${p.y.toFixed(3)},${p.z.toFixed(3)}]m rpy=[${p.roll.toFixed(2)},${p.pitch.toFixed(2)},${p.yaw.toFixed(2)}]deg`);lastHealthLogAt=now;}
}
...
log('INFO',`Build=20260925.27 · absolute T265 map prior`);
