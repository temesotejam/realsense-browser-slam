export class RealSenseSource {
  ...
  async start(depthId,rgbId=''){
    await this.stop();
    this.depthStream=await navigator.mediaDevices.getUserMedia({video:{deviceId:{exact:depthId},width:{ideal:640},height:{ideal:480},frameRate:{ideal:30}},audio:false});
    this.depthVideo=makeVideo(this.depthStream);await waitVideo(this.depthVideo);await this.depthVideo.play();
    ...
    if(rgbId){try{this.rgbStream=await navigator.mediaDevices.getUserMedia({video:{deviceId:{exact:rgbId},width:{ideal:640},height:{ideal:480},frameRate:{ideal:30}},audio:false});this.rgbVideo=makeVideo(this.rgbStream);await waitVideo(this.rgbVideo);await this.rgbVideo.play();}catch(e){console.warn('RGB optional stream unavailable',e);this.rgbStream=null;this.rgbVideo=null;}}
    this.initDepthGl();this.lastMediaTime=null;this.cameraFps=0;
    return {label:this.label,width:this.width,height:this.height,settings,rgb:!!this.rgbVideo};
  }
  initDepthGl(){
