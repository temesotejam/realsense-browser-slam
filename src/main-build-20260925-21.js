...
const worker=new Worker(new URL('./slam-worker-build-20260925-21.js?v=20260925.21',import.meta.url),{type:'classic'});
...
let latestT265=null,depthFps=0,lastDepthArrival=0,depthFrames=0,hubOnline=false,hubDepthActive=false,hubT265Active=false,autoStarted=false,lastOrientationBuild='',warnedOrientationBuild=false;
...
function startSlam(){
  if(running)return;
  running=true;busy=false;dropped=0;lastSent=0;originMatrix=null;path=[];keyframePath=[];trajectoryRows=[];lastTrackingState='';lastHealthLogAt=0;lastLoggedLoops=0;lastLoggedRelocalized=0;lastLoggedKeyframes=0;lastLoggedMapMatches=0;lastLoggedMapRejects=0;lastLoggedMapRepairs=0;
  ...
  log('INFO',`SLAM processing started automatically. T265=${latestT265?'available':'waiting'}, RGB=${rgbSource.active?'ON':'OFF'}`);
  const k=scaledK();log('INFO',`SLAM intrinsics 320x240: fx=${k.fx.toFixed(3)} fy=${k.fy.toFixed(3)} cx=${k.cx.toFixed(3)} cy=${k.cy.toFixed(3)}`);
}
function pauseSlam(){
...
log('INFO',`Build=20260925.21 · Sensor Hub + T265 rotation fusion`);
