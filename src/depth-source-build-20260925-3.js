export class RealSenseSource {
  ...
  async start(depthId,rgbId='',depthGroupId=''){
    await this.stop();
    const supported=navigator.mediaDevices?.getSupportedConstraints?.()||{};
    this._log('INFO',`Depth API support: videoKind=${!!supported.videoKind} groupId=${!!supported.groupId}`);
    const depthAttempts=[];
    if(supported.videoKind) depthAttempts.push({name:'deviceId+videoKind-depth',video:{deviceId:{exact:depthId},videoKind:{exact:'depth'}}});
    if(supported.videoKind&&depthGroupId) depthAttempts.push({name:'groupId+videoKind-depth',video:{groupId:{exact:depthGroupId},videoKind:{exact:'depth'}}});
    if(supported.videoKind) depthAttempts.push({name:'videoKind-depth',video:{videoKind:{exact:'depth'}}});
    depthAttempts.push(
      ...
      {name:'640x480@30-exact',video:{deviceId:{exact:depthId},width:{exact:640},height:{exact:480},frameRate:{exact:30}}},
      {name:'640x480@15',video:{deviceId:{exact:depthId},width:{ideal:640},height:{ideal:480},frameRate:{ideal:15}}},
